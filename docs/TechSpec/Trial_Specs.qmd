...
This document is available at the N-ALB MSE homepage.
...
The total allowable catch (TAC) for the next management period ($t$) is assumed constant, unless a reference value ($I_{G}$) derived from a group of indices of abundance in the last two years falls below a reference value $I_{ref}=0.91$:
...
In addition, $TAC_{t} = TAC_{t-1}$ if the difference between $TAC_{t}$ and $TAC_{t-1}$ was larger than 15%.
...
and $I_{ref}=1$. In addition, $TAC_{t} = TAC_{max}$ when $TAC_{t} > TAC_{max}$, where $TAC_{max} = 50,000$ tonnes. Also, $TAC_{t} = TAC_{t-1}$ if the difference between $TAC_{t}$ and $TAC_{t-1}$ was larger than 15%.
...
In the HCR, $F_{tgt}$ and $B_{thr}$ need to be defined, and the following options were tested:
