...
- Changes in $R_0$: decrease or increase in the $R_0$ parameter of the Beverton-Holt relationship by 20% in the projection period from the estimated values in the Reference OMs.
...
where $TAC_{ct}=42,000$ tonnes (@fig-pcc). $I_{G}$ was calculated as follows:
...
In addition, the TAC variation between consecutive management periods $TAC_{t+1}$ and $TAC_{t}$ could not exceed 15%.
...
### Index-based {#sec-mp-emp}

The total allowable catch (TAC) for the next management period ($t+1$) is calculated from the TAC in the current management period $t$ and a factor $J_{rat}$ derived from a selected indices (see @sec-mp-pcc). First, we calculated a *combined* index of abundance ($J_Y$) for every year $Y$, where $Y\in \{y-1,...,y-6\}$:

<!-- $$J_Y = \frac{\sum_{i=i}^{n_i}I_{i,Y}}{n_i}$${#eq-emp1} -->

$$J_Y = \frac{\sum_{i=1}^{n_i}I_{i,Y}w_{i,Y}}{\sum_{i=1}^{n_i}w_{i,Y}}$${#eq-emp}

where $n_i$ represents the number of selected indices, $I_{i,Y}$ represents the value of index $i$ in year $Y$, and $w_{i,Y}$ is the weighting factor. $w_{i,Y}$ is derived from the standard deviation ($\sigma$) and temporal autocorrelation ($\rho$) of indices residuals:

$$w_{i,Y}=\frac{1}{\sqrt{\sigma_i/(1-\rho_i)}}$$

Once $J_Y$ is calculated, we then derive the ratio of change in $J$ between two consecutive periods in the previous six years (@fig-Jrat):

$$J_{rat} = \frac{J_{y-1}+J_{y-2}+J_{y-3}}{J_{y-4}+J_{y-5}+J_{y-6}}$$

![Example of the derivation of $J_Y$ from a set of indices. The red line represents $J_Y$ for the previous six years.](../figures/Emp_calculation.png){#fig-Jrat width="55%"}

Finally, the TAC for the next management period is obtained:

$$TAC_{t+1} = J_{rat} TAC_t $$

In addition, we imposed two types of restriction on the TAC variation between consecutive management periods $TAC_{t+1}$ and $TAC_{t}$:

- Maximum increase of 25% and decrease of 20%, or
- Maximum increase and decrease of 10%
...
- Maximum increase or decrease of 10%, or
