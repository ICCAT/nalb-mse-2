...
The total allowable catch (TAC) for the next management period ($t$) is assumed constant, unless a reference value ($I_{G}$) derived from a group of indices of abundance in the last two years falls below a reference value $I_{ref}=0.91$ (value in 2010, when $SSB>SSB_{msy}$):
...
where $TAC_{max}=42,000$ tonnes. $I_{G}$ was calculated as follows:
...
For this case, the indices of abundance $I_i$ that we used were:

-   Baitboat
-   Japan Longline South
-   Taiwan Longline North
-   Taiwan Longline South
-   U.S. Longline North
-   U.S. Longline South.

In addition, $TAC_{t} = \pm 15\% TAC_{t-1}$ if the difference between $TAC_{t}$ and $TAC_{t-1}$ was larger than 15% and $I_G < I_{ref}$.
...
where we assumed $\alpha=0.15$ and $\beta=0.15$. In this case, $I_{G}$ was calculated as:
...
and $I_{ref}=1$ (value in 2012, when $SSB>SSB_{msy}$). In addition, $TAC_{t} = TAC_{max}$ when $TAC_{t} > TAC_{max}$, assuming that $TAC_{max} = 50,000$ tonnes. Also, $TAC_{t} = \pm 15\% TAC_{t-1}$ if the difference between $TAC_{t}$ and $TAC_{t-1}$ was larger than 15%.
