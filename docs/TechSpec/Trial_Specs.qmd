...
- Maximum catch ($TAC_{max}$): maximum annual catch of 100,000 tonnes.
...
timeline
    ...
    section 2022 - 2050
        Simulation period : - Catches from HCR (Model-based CMPs) <br> - CPUE from OEM <br> - Evaluate CMPs
```
...
### Pseudo-constant catch (PCCatch)

The total allowable catch (TAC) for the next management period ($t$) is assumed constant, unless a reference value ($I_{G}$) derived from a group of indices of abundance in the last two years falls below a reference value $I_{ref}=0.91$ (value observed in 2010, when $SSB>SSB_{msy}$):
...
where $n_i$ is the number of selected indices of abundance and $I^*_i$ was calculated for every selected index of abundance $I_i$ using information from the last two years ($y$):

$$I^*_i = \frac{I_{i,y}+I_{i,y-1}}{2}$$
...
In addition, the TAC variation between consecutive management periods $TAC_{t}$ and $TAC_{t-1}$ could not exceed 15% when $I_G < I_{ref}$.

![Derivation of the TAC in the following management period ($TAC_t$) in the pseudoconstant catch MP.](../figures/pccatch.png){#fig-pcc width="55%"}

### Index-based

TAC for the next management period $t$ is calculated as a function of the previous TAC ($TAC_{t-1}$):

$$TAC_t =
\begin{cases}
TAC_{t-1}, & \text{if }(1-\alpha) \leq I_{G} \leq (1+\alpha) \\
(1+\beta)TAC_{t-1}, & \text{if }I_{G} > (1+\alpha) \\
(1-\beta)TAC_{t-1}, & \text{if }I_{G} < (1-\alpha)
\end{cases}$$

where we assumed $\alpha=0.15$ and $\beta=0.15$ (@fig-ibased). In this case, $I_{G}$ was calculated as:
...
and $I_{ref}=1$ (value observed in 2012, when $SSB>SSB_{msy}$). In addition, $TAC_{t} = TAC_{max}$ when $TAC_{t} > TAC_{max}$, assuming that $TAC_{max} = 50,000$ tonnes. Also, the TAC variation between consecutive management periods $TAC_{t}$ and $TAC_{t-1}$ could not exceed 15%.

![Derivation of the TAC in the following management period ($TAC_t$) in the index-based MP.](../figures/index_based.png){#fig-ibased width="55%"}
...
![Example of harvest control rule (HCR) in model-based MPs.](../figures/hcr_example.png){#fig-hcr width="55%"}

The management advice is given every three years (*management period*) and the total allowable catch (TAC) is derived from a harvest control rule (HCR, @fig-hcr). 

In the HCR, $F_{tgt}$ and $B_{thr}$ need to be defined, and the following options were tested (@fig-combs):

- $F_{tgt}=0.8F_{msy}$ and $B_{thr}=B_{msy}$
- $F_{tgt}=F_{msy}$ and $B_{thr}=B_{msy}$

![Example of harvest control rule (HCR).](../figures/hcr_combs_2.png){#fig-combs width="55%"}

In addition, $B_{lim}=0.4\times B_{msy}$ and $F_{mim}=0.1\times F_{msy}$ for all cases. These MPs were divided into two groups, depending on the maximum allowed variation between $TAC_{t}$ and $TAC_{t-1}$: 
