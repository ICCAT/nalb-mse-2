...
### Empirical {#sec-mp-emp}

The total allowable catch (TAC) for the next management period ($t+1$) is calculated from the TAC in the current management period $t$ and a factor $J_{rat}$ derived from a selected indices of abundance:
...
First, we calculated a *combined* index of abundance ($J_Y$) for every year $Y$, where $Y\in \{y-1,...,y-6\}$:

$$J_Y = \frac{\sum_{i=i}^{n_i}I_{i,Y}}{n_i}$${#eq-emp1}

where $n_i$ represents the number of selected indices, and $I_{i,Y}$ represents the value of index $i$ in year $Y$. 

An alternative way of calculating $J_Y$ in @eq-emp1 was also explored, where each index $I_i$ was weighted by a factor $w_{i,Y}$:

$$J_Y = \frac{\sum_{i=i}^{n_i}I_{i,Y}w_{i,Y}}{\sum_{i=i}^{n_i}w_{i,Y}}$${#eq-emp2}

In @eq-emp2, $w_{i,Y}$ is derived from the standard deviation ($\sigma$) and temporal autocorrelation ($\rho$) of indices residuals:
...
- Maximum increase and decrease of 25%
- Maximum increase and decrease of 10%

Therefore, four empirical HCR were obtained.
